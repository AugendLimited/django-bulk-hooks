...
class RegistryTestCase(TestCase):
    ...
    def test_hook_registration(self):
        ...
        for handler_cls, method_name, condition, priority in before_create_hooks:
            if (
                handler_cls == BasicRegistryHooks
                and method_name == "before_create_basic"
            ):
                hook_found = True
                break
    ...
    def test_hook_priority_in_registry(self):
        ...
        for handler_cls, method_name, condition, priority in after_create_hooks:
            if handler_cls == BasicRegistryHooks and method_name == "after_create_high":
    ...
    def test_hook_condition_in_registry(self):
        ...
        for handler_cls, method_name, condition, priority in before_update_hooks:
            if (
                handler_cls == BasicRegistryHooks
                and method_name == "before_update_conditional"
            ):
        ...
        self.assertTrue(
            hook_found, "before_update_conditional hook not found in registry"
        )
...
class DecoratorTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Clear the global hook registry for clean slate within isolation
    ...
    def test_decorator_with_complex_conditions(self):
        ...
        class ComplexConditionHooks(Hook):
            @hook(
                BEFORE_UPDATE,
                model=RegistryTestModel,
                condition=(HasChanged("value") & IsEqual("status", "active"))
                | IsEqual("name", "special"),
            )
    ...
    def test_decorator_preserves_method_metadata(self):
        ...
        self.assertEqual(
            MetadataHooks.documented_hook.__doc__, "This is a documented hook method."
        )

        # Check that method name is preserved
        self.assertEqual(MetadataHooks.documented_hook.__name__, "documented_hook")
    ...
    def test_multiple_decorators_same_method(self):
        ...
        multi_calls = [
            call for call in registry_state["hook_calls"] if call == "multi_event_hook"
        ]
...
class RegistryEdgeCasesTestCase(TestCase):
    ...
    def test_registry_memory_usage(self):
        ...
        for i in range(50):  # Adjust based on test environment
            ...
            hook_method = hook(BEFORE_CREATE, model=RegistryTestModel)(
                make_hook_method(i)
            )
        ...
        dynamic_hooks_count = sum(
            1
            for _, method_name, _, _ in before_hooks
