    ...
    def validate_account_creation(self, new_records, old_records):
        ...
        for account in new_records:
            if account.balance < 0:
                raise ValidationError(f"Account {account.name} cannot have negative balance")
    ...
    def validate_account_update(self, new_records, old_records):
        ...
        for account in new_records:
            if account.balance < -1000:
                raise ValidationError(f"Account {account.name} balance cannot be below -1000")
...
class ProductHooks(Hook):
    ...
    @hook(BEFORE_UPDATE, model=Product, condition=HasChanged('category') & IsEqual('in_stock', True))
    def before_category_change_in_stock(self, new_records, old_records):
        ...
        hook_state['product_events'].append(('category_change_in_stock', len(new_records)))
...
class BulkHooksTestCase(TestCase):
    ...
    def test_bulk_delete_hooks(self):
        ...
        # Verify hooks were called
        expected_calls = [
            'account_before_delete',
            'account_after_delete'
        ]
        self.assertEqual(hook_state['calls'], expected_calls)
    ...
    def test_individual_model_save_update_hooks(self):
        ...
        account = Account.objects.create(name='Test Account', balance=100, created_by=self.user)
        reset_hook_state()
...
class HookConditionsTestCase(TestCase):
    ...
    def test_is_greater_than_condition(self):
        ...
        product = Product.objects.create(name='Test Product', price=50, category='electronics')
        reset_hook_state()
    ...
    def test_and_condition(self):
        ...
        product = Product.objects.create(name='Test Product', price=50, category='electronics', in_stock=True)
        reset_hook_state()
...
class EdgeCasesTestCase(TestCase):
    ...
    def test_non_existent_field_update(self):
        ...
        # This should not cause errors but also should not trigger field-specific conditions
    ...
    def test_hook_with_related_field_access(self):
        ...
        account = Account.objects.create(name='Main Account', balance=100, created_by=self.user)
        transaction = Transaction.objects.create(
            account=account,
            amount=50,
            description='Test transaction'
